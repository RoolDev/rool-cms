...
function initializeExternalInterfaces() {
  'use strict';
  window.HabboFlashClient.init(document.getElementById('client'));
}
!(function () {
  'use strict';
  var n = '*';
    ...
    (window.FlashExternalInterface = {}),
      ...
      (window.FlashExternalInterface.heartBeat = function () {
        (window.HabboFlashClient.started = !0),
          window.MainApp.postMessage({ call: 'heartbeat' });
      }),
      ...
      (window.FlashExternalInterface.loadConversionTrackingFrame = function () {
        ...
        $('#conversion-tracking').attr(
          'src',
          '/client/' + n + '/conversion-tracking'
        );
      }),
      (window.FlashExternalInterface.logCrash = function (n) {
        (window.HabboFlashClient.started = !0),
          ...
          window.HabboWebApi.logCrash(n, function (n) {
            n &&
              window.FlashExternalInterface.track(
                'log',
                'fatal',
                "Can't log login crash: " + n
              );
          });
      }),
      ...
      (window.FlashExternalInterface.logLoginStep = function (e, a) {
        window.FlashExternalInterface.track('clientload', e, a),
          (window.HabboFlashClient.started = !0),
          n ||
            'client.init.core.running' !== e ||
            ((n = !0), window.MainApp.postMessage({ call: 'hotel-ready' })),
          window.HabboWebApi.logLoginStep(e, a, function (n) {
            n &&
              window.FlashExternalInterface.track(
                'log',
                'error',
                "Can't log login step: " + n
              );
          });
      }),
      ...
      (window.FlashExternalInterface.openPage = function (n) {
        (n = window.HabboPageTransformer.translate(n)),
          ...
          window.MainApp.postMessage({ call: 'open-page', target: n });
      }),
      ...
      (window.FlashExternalInterface.openMinimail = function (n) {
        window.HabboTracking.track('minimail', 'open', n),
          window.MainApp.postMessage({ call: 'open-page', target: '/' });
      }),
  ...
  (function () {
    ...
    window.addEventListener('message', n, !1),
      (window.HabboFlashClient = {
        started: !1,
        init: function (n) {
          window.HabboTracking.track(
            'clientload',
            'starting',
            'Initalizing Habbo Client.'
          ),
            window.FlashExternalInterface.logLoginStep('web.view.start'),
            n ||
              (console.error(
                "Invalid FlashClient. Can't use JS->Flash interface."
              ),
              window.FlashExternalInterface.logLoginStep('web.flash_missing')),
            (t = n),
            (window.HabboFlashClient.flashInterface = n),
            setTimeout(function () {
              window.HabboFlashClient.started ||
                window.FlashExternalInterface.logLoginStep(
                  'client.init.swf.error'
                );
            }, 3e4);
        },
      });
  ...
  (function () {
    ...
    var n = function (n, e, a) {
      'console' in window &&
        'log' in console &&
        console.log(
          'action = [' + n + '], label = [' + e + '], value = [' + a + ']'
        );
      if (n == 'navigator' && e == 'private') {
        roomId = a;
      }
    };
    ...
    (window.NewUserReception = {}),
      (window.NewUserReception.checkName = function (n) {
        console.log('Checking name: "' + n + '"...'),
          window.HabboWebApi.checkName(n, function (e, a) {
            return e
              ? (console.error('Checking name failed!'),
                void window.HabboFlashClient.flashInterface.newUserReceptionCheckNameFailed(
                  n
                ))
              : void window.HabboFlashClient.flashInterface.newUserReceptionCheckNameResponse(
                  n,
                  a.code,
                  a.validationResult || [],
                  a.suggestions
      ...
      (window.NewUserReception.chooseRoom = function (n) {
        console.log('Choosing room: "' + n + '"...'),
          window.HabboWebApi.selectRoom(n, function (n) {
            return n
              ? (console.error('Choosing room failed!'),
                void window.HabboFlashClient.flashInterface.newUserReceptionChooseRoomFailed())
      ...
      (window.NewUserReception.claimName = function (n) {
        console.log('Claiming name: "' + n + '"...'),
          window.HabboWebApi.claimName(n, function (e, a) {
            return e
              ? (console.error('Claiming name failed!'),
                void window.HabboFlashClient.flashInterface.newUserReceptionClaimNameFailed(
                  n
                ))
      ...
      (window.NewUserReception.saveOutfit = function (n, e) {
        console.log('Saving outfit: "' + n + '" - "' + e + '"...'),
          window.HabboWebApi.saveFigure(n, e, function (n, a) {
            return n
              ? (console.error('Saving outfit failed!'),
                void window.HabboFlashClient.flashInterface.newUserReceptionSaveOutfitFailed())
              : (window.HabboFlashClient.flashInterface.newUserReceptionSaveOutfitResponse(
                  a.figureString,
                  e,
                  'OK'
                ),
                void window.MainApp.postMessage({
                  call: 'update-figure',
    ...
    function n() {
      window._sp_video = new window.SPONSORPAY.Video.Iframe({
        ...
        callback_on_start: function (n) {
          window.HabboFlashClient.flashInterface.sponsorPayOnStart(
            n.icon_small
          );
        },
    ...
    window.SponsorPay = {
      loadIntegration: function (e) {
        ...
        a.setAttribute('type', 'text/javascript'),
          a.setAttribute('src', '//be.sponsorpay.com/assets/web_client.js'),
  ...
  (function () {
    ...
    function n() {
      ...
      (t.className = 'ssaTopBarCloseButton'),
        t.setAttribute('onClick', 'SSA_CORE.close("ssaBrandConnect")'),
        (t.innerHTML = '');
      var i = document.createElement('span');
      (i.className = 'ssaTopBarTextSpan'),
        (i.innerHTML = e),
        a.appendChild(t),
        a.appendChild(i),
        n.appendChild(o),
      ...
      (l.className = 'ssaBottomBarInnerRight'),
        s.appendChild(l),
        s.appendChild(r);
    }
    function e(n) {
      n && n.length > 0
        ? window.HabboFlashClient.flashInterface.supersonicAdsOnCampaignsReady(
            n.length.toString()
          )
        : window.HabboFlashClient.flashInterface.supersonicAdsOnCampaignsReady(
            '0'
    ...
    var s =
        'https://a248.e.akamai.net/ssastatic.s3.amazonaws.com/inlineDelivery/delivery.min.gz.js',
      r = window.flashvars || {},
    ...
    l
      ? (r.supersonic_demo_campaigns && (w.demoCampaigns = 1),
        (w.applicationUserId = r.supersonic_admin_id || r.account_id))
      : (w.applicationUserId = r.account_id),
      (window.supersonicAdsCamapaignEngage = n),
      (window.supersonicAdsOnCampaignsReady = e),
      (window.supersonicAdsOnCampaignOpen = a),
      (window.supersonicAdsOnCampaignClose = o),
      (window.supersonicAdsOnCampaignCompleted = t),
      ...
      (window.TargetedWebOffer.checkOffer = function () {
        console.log('Checking for offer...'),
          window.HabboShopApi.checkOffer(function (n, e) {
            return n
              ? void window.HabboFlashClient.flashInterface.targetedWebOfferCheckFailed()
              : void window.HabboFlashClient.flashInterface.targetedWebOfferCheckResponse(
                  e
                );
          });
      });
